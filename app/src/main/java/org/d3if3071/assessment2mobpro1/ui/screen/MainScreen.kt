...
import org.d3if3071.assessment2mobpro1.navigation.Screen
import org.d3if3071.assessment2mobpro1.ui.theme.Assessment2Mobpro1Theme
import org.d3if3071.assessment2mobpro1.util.SettingsDataStore
...
@Composable
fun ListItem(sampah: Sampah, onClick: () -> Unit) {
    ...
    Column(
        modifier = Modifier
            ...
            .clickable {
                onClick()
                showToast(context, R.string.x_diklik, sampah.namaPengguna)}
            .padding(16.dp),
        verticalArrangement = Arrangement.spacedBy(8.dp)
    ){
        Text(
            text = sampah.namaPengguna,
            maxLines = 1,
            overflow = TextOverflow.Ellipsis,
            fontWeight = FontWeight.Bold
        )
        Text(
            text = sampah.kategoriSampah,
            maxLines = 1,
            overflow = TextOverflow.Ellipsis
        )
        Text(
            text = sampah.berat + " kg",
            maxLines = 1,
            overflow = TextOverflow.Ellipsis
        )
        Text(
            text = sampah.tanggalPenjemputan,
            maxLines = 1,
            overflow = TextOverflow.Ellipsis
        )
        Text(text = sampah.alamat)
    }
}

@Composable
fun GridItem(sampah: Sampah, onClick: () -> Unit) {
    val context = LocalContext.current
